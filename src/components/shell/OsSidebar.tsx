...
import { getPanelsBySection } from '@/lib/ui-registry';
...
const OsSidebar = memo(function OsSidebar() {
  ...
  const sectionEntries = useMemo(() => {
    const map = getPanelsBySection();
    const order = ['KERNEL', 'PRINCIPAL', 'LOGISTIQUE', 'ALIMENTATION', 'ESPRIT ET CULTURE', 'SPORT ET SANTÉ', 'AUTRE'];
    return order.filter(s => map.has(s)).map(s => [s, map.get(s)!] as const);
  }, []);

  useEffect(() => {
    if (expandedSections.length === 0) {
      setExpandedSections(sectionEntries.map(([s]) => s));
    }
  }, [sectionEntries]);
  ...
  useEffect(() => {
    if (sidebarRef.current) {
      gsap.to(sidebarRef.current, { width: isExpanded ? 190 : 70, duration: 0.25, ease: 'power2.out' });
    }
  }, [isExpanded]);
  ...
  useEffect(() => {
    if (logoTextRef.current) {
      gsap.to(logoTextRef.current, {
        opacity: isExpanded ? 1 : 0, width: isExpanded ? 'auto' : 0, duration: 0.2,
          ...
          <div key={section} className="mb-2">
            ...
            >
              {isExpanded ? (
                <>
                  <span>{section}</span>
                  <ChevronDown className={cn('w-4 h-4 transition-transform', expandedSections.includes(section) ? '' : '-rotate-90')} />
                </>
              ) : (
        ...
        <div className={cn("flex items-center gap-2 px-3 py-2", !isExpanded && "justify-center")}>
          ...
          {isExpanded && (
            <span className="text-[10px] text-slate-500">
              {kernelStatus === 'connected' ? 'Kernel connecté' : kernelStatus === 'connecting' ? 'Connexion…' : 'Hors ligne'}
            </span>
          )}
        </div>
      </div>
      ...
      <div className="px-4 py-2 border-t border-white/5">
        <button
          ...
          className={cn(
            ...
            activePanel === 'settings' ? 'bg-emerald-500/15 text-emerald-400' : 'text-slate-400 hover:bg-white/5 hover:text-white',
            !isExpanded && 'justify-center'
          )}
        ...
        <div className={cn("flex items-center gap-3", !isExpanded && "justify-center")}>
          ...
          {isExpanded && (
            <div ref={userTextRef} className="flex-1 min-w-0 overflow-hidden">
              ...
              <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold bg-emerald-500/15 text-emerald-400">PREMIUM</span>
            </div>
          )}
        </div>
      </div>
